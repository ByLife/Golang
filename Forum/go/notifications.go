...
func Getprofileinfo(w http.ResponseWriter, r *http.Request) { //gets notification for a user
	...
	for rows.Next() {
		if err := rows.Scan(&Inter.Postid, &Inter.Uuid, &Inter.Karma); err != nil {
			return
		}
...
func Getnotifications(w http.ResponseWriter, r *http.Request) { //gets notification for a user NOT YET FINISHED
	if !Isregistered(w, r) {
		return
	}
	db, _ := sql.Open("sqlite3", "./database.db")
	...
	for rows.Next() {
		...
		var Inter Interaction
		for rows2.Next() {
			if err := rows.Scan(&Inter.Postid, &Inter.Uuid, &Inter.Karma, &Inter.Date); err != nil {
				// fmt.Print(err)
				defer rows2.Close()
				defer rows.Close()
				return
