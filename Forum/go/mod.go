...
func Getadmin(w http.ResponseWriter, r *http.Request) { //get the list of admins
	var mod Mod
	db, _ := sql.Open("sqlite3", "./database.db")
	rows, _ := db.Query("SELECT uuid, username, email, creationdate, level, karma FROM users WHERE level = 2 OR level = 3")
	defer rows.Close()
	templ.Mods = nil
	for rows.Next() {
		if err := rows.Scan(&mod.Uuid, &mod.Username, &mod.Email, &mod.Creationdate, &mod.Level, &mod.Karma); err != nil {
			return
		}
